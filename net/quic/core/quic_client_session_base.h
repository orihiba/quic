...
class QuicClientPromisedInfo2;
class QuicClientPushPromiseIndex;
class QuicClientPushPromiseIndex2;
...
using QuicPromisedByUrlMap2 =
std::unordered_map<std::string, QuicClientPromisedInfo2*>;
...
public:
	...
	// Session retains ownership.
	QuicClientPromisedInfo2* GetPromisedByUrl(const std::string& url);
	// Session retains ownership.
	QuicClientPromisedInfo2* GetPromisedById(const QuicStreamId id);

	//
	QuicNormalStream* GetPromisedStream(const QuicStreamId id);

	// Removes |promised| from the maps by url.
	void ErasePromisedByUrl(QuicClientPromisedInfo2* promised);

	// Removes |promised| from the maps by url and id and destroys
	// promised.
	virtual void DeletePromised(QuicClientPromisedInfo2* promised);
	...
	QuicClientPushPromiseIndex2* push_promise_index() {
		return push_promise_index_;
	}
...
private:
	// For QuicSpdyClientStream to detect that a response corresponds to a
	// promise.
	using QuicPromisedById2Map =
		std::unordered_map<QuicStreamId, std::unique_ptr<QuicClientPromisedInfo2>>;
	...
	// promise info when the response headers of the promised stream
	// arrive.
	QuicClientPushPromiseIndex2* push_promise_index_;
	QuicPromisedById2Map promised_by_id_;
	QuicStreamId largest_promised_stream_id_;

private:
	
	DISALLOW_COPY_AND_ASSIGN(QuicNormalClientSessionBase);
};
