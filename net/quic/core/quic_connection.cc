...
class IdleTimeoutAlarmDelegate : public QuicAlarm::Delegate {
public:
	explicit IdleTimeoutAlarmDelegate(QuicConnection* connection)
		: connection_(connection) {}

	void OnAlarm() override { connection_->CheckForIdleTimeout(); }
...
class PeekAlarmDelegate : public QuicAlarm::Delegate {
public:
	explicit PeekAlarmDelegate(QuicConnection* connection)
		: connection_(connection) {}

	void OnAlarm() override { connection_->OnPeekTimeout(); }

private:
	QuicConnection* connection_;

	DISALLOW_COPY_AND_ASSIGN(PeekAlarmDelegate);
};
...
void QuicConnection::UpdateFecCofiguration(QuicPacketCount packets_received)
{
	...
	if (loss_rate <= 0.1) {			// 0
		...
		loss_rate_index = 1;
	} else if (loss_rate < 7) {	// 3
		loss_rate_index = 2;
	} else if (loss_rate < 11.5) {	// 9
		loss_rate_index = 3;
...
bool QuicConnection::OnAckFrame(const QuicAckFrame& incoming_ack) {
  ...
  // check if all FEC group packets were received successfully, so there is no need to send more FEC packets
...
void QuicConnection::OnFecProtectedPayload(StringPiece payload) {
	...
	if (group != nullptr) {
		group->UpdateReceivedList(last_decrypted_packet_level_, last_header_, payload, false);
	}
}
...
void QuicConnection::OnFecData(StringPiece redundancy) {
	...
	DCHECK_NE(0u, last_header_.fec_group);
	QuicFecGroup* group = GetFecGroup();
	if (group != nullptr && group->NumReceivedPackets() != 0) {
...
// //	 peek_alarm_->Set(clock_->ApproximateNow() + peek_active_time_);
 ...
 //                                                listener);
 // }
  return packet_generator_.ConsumeData(id, iov, offset, fin, fec_protection, listener);
}
...
void QuicConnection::WriteQueuedPackets() {
  ...
  QueuedPacketList::iterator packet_iterator = queued_packets_.begin();
  while (packet_iterator != queued_packets_.end() && /*should_write_ && */
         WritePacket(&(*packet_iterator))) {
	  DVLOG(1) << "SENTT " << packet_iterator->packet_number;
    delete[] packet_iterator->encrypted_buffer;
    QuicUtils::ClearSerializedPacket(&(*packet_iterator));
    packet_iterator = queued_packets_.erase(packet_iterator);
  }
...
void QuicConnection::WritePendingRetransmissions() {
  ...
  // written.
  while (sent_packet_manager_->HasPendingRetransmissions() /*&& should_write_*/) {
...
//  DCHECK_LE(encrypted_length, packet_generator_.GetCurrentMaxPacketLength());  // HIBA removed because added padding bytes to ensure diversion by 8
  ...
  DVLOG(2) << ENDPOINT << "packet(" << packet_number << "): " << std::endl
           << QuicUtils::HexDump(
                 StringPiece(packet->encrypted_buffer, encrypted_length)) << std::endl;
...
void QuicConnection::CheckForIdleTimeout()
{
	return; // TODO HIBA
	QuicTime now = clock_->ApproximateNow();
	QuicTime::Delta idle_duration = now - time_of_last_received_packet_;
	if (idle_duration >= new_idle_network_timeout_) {
