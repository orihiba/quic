...
size_t QuicFramer::GetSerializedFrameLength(
  ...
  size_t frame_len =
      ComputeFrameLength(frame, last_frame, is_in_fec_group, packet_number_length);
...
QuicPacket* QuicFramer::BuildFecPacket(const QuicPacketHeader& header,
	...
	return new QuicPacket(buffer.release(), len, true,
		...
		false, // not a version packet //header.public_header.version_flag,
		header.public_header.multipath_flag, // not sure
		false, //????? TODO not sure
...
bool QuicFramer::ProcessDataPacket(QuicDataReader* encrypted_reader,
  ...
  //if (quic_version_ <= QUIC_VERSION_33) { // HIBA - on my version always check authentication header
...
bool QuicFramer::ProcessRevivedPacket(QuicPacketHeader* header,
	...
	DVLOG(1) << "Reviving packet number: " << header->packet_number;
	if (!visitor_->OnPacketHeader(*header)) {
		return true;
	}
