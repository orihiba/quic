...
void QuicNormalClientSessionBase::OnConfigNegotiated() {
	QuicSession::OnConfigNegotiated();
}

void QuicNormalClientSessionBase::OnCryptoHandshakeEvent(CryptoHandshakeEvent event) {
	QuicSession::OnCryptoHandshakeEvent(event);
}

void QuicNormalClientSessionBase::OnPromiseHeaders(QuicStreamId stream_id,
	StringPiece headers_data) {
	//QuicNormalStream* stream = GetOrCreateDynamicStream(stream_id);
	//if (!stream) {
	//	// It's quite possible to receive headers after a stream has been reset.
	//	return;
	//}
...
void QuicNormalClientSessionBase::OnInitialHeadersComplete(
	QuicStreamId stream_id,
	const SpdyHeaderBlock& response_headers) {
	// Note that the strong ordering of the headers stream means that
	// QuicSpdyClientStream::OnPromiseHeadersComplete must have already
...
void QuicNormalClientSessionBase::OnPromiseHeadersComplete(
	QuicStreamId stream_id,
	QuicStreamId promised_stream_id,
	size_t frame_len) {
	//if (promised_stream_id != kInvalidStreamId &&
...
void QuicNormalClientSessionBase::OnPromiseHeaderList(
	QuicStreamId stream_id,
	QuicStreamId promised_stream_id,
	size_t frame_len,
	const QuicHeaderList& header_list) {
...
void QuicNormalClientSessionBase::HandlePromised(QuicStreamId /* associated_id */,
	QuicStreamId id,
	const SpdyHeaderBlock& headers) {
	//// Due to pathalogical packet re-ordering, it is possible that
	//// frames for the promised stream have already arrived, and the
...
QuicClientPromisedInfo2* QuicNormalClientSessionBase::GetPromisedByUrl(
	const string& url) {
	QuicPromisedByUrlMap2::iterator it =
		push_promise_index_->promised_by_url()->find(url);
	if (it != push_promise_index_->promised_by_url()->end()) {
		return it->second;
	}
...
QuicClientPromisedInfo2* QuicNormalClientSessionBase::GetPromisedById(
	const QuicStreamId id) {
	QuicPromisedById2Map::iterator it = promised_by_id_.find(id);
	if (it != promised_by_id_.end()) {
		return it->second.get();
	}
	return nullptr;
...
QuicNormalStream* QuicNormalClientSessionBase::GetPromisedStream(
	const QuicStreamId id) {
	if (IsClosedStream(id)) {
		return nullptr;
	}
	DynamicStreamMap::iterator it = dynamic_streams().find(id);
	if (it != dynamic_streams().end()) {
		return static_cast<QuicNormalStream*>(it->second);
	}
	QUIC_BUG << "Open promised stream " << id << " is missing!";
	return nullptr;
}

void QuicNormalClientSessionBase::DeletePromised(QuicClientPromisedInfo2* promised) {
	push_promise_index_->promised_by_url()->erase(promised->url());
	// Since promised_by_id_ contains the unique_ptr, this will destroy
	// promised.
	promised_by_id_.erase(promised->id());
...
void QuicNormalClientSessionBase::OnPushStreamTimedOut(QuicStreamId stream_id) {}

void QuicNormalClientSessionBase::ResetPromised(QuicStreamId id,
	QuicRstStreamErrorCode error_code) {
	SendRstStream(id, error_code, 0);
	if (!IsOpenStream(id)) {
		MaybeIncreaseLargestPeerStreamId(id);
