...
#ifndef NET_QUIC_CRYPTO_MY_DECRYPTER_H_
#define NET_QUIC_CRYPTO_MY_DECRYPTER_H_
...
class NET_EXPORT_PRIVATE MyDecrypter : public QuicDecrypter {
 public:
	 enum {
		 kAuthTagSize = 12,
	 };
  MyDecrypter();
  MyDecrypter(const EVP_AEAD* aead_alg,
                    size_t key_size,
                    size_t auth_tag_size,
                    size_t nonce_prefix_size);
  ~MyDecrypter() override;
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(MyDecrypter);
};

}  // namespace net

#endif  // NET_QUIC_CRYPTO_MY_DECRYPTER_H_
