...
void QuicNormalServerSessionBase::OnCongestionWindowChange(QuicTime now) {
	if (!bandwidth_resumption_enabled_) {
		return;
	}
	// Only send updates when the application has no data to write.
...
bool QuicNormalServerSessionBase::ShouldCreateIncomingDynamicStream(QuicStreamId id) {
	if (!connection()->connected()) {
		QUIC_BUG << "ShouldCreateIncomingDynamicStream called when disconnected";
		return false;
	}
...
bool QuicNormalServerSessionBase::ShouldCreateOutgoingDynamicStream() {
	if (!connection()->connected()) {
		QUIC_BUG << "ShouldCreateOutgoingDynamicStream called when disconnected";
		return false;
	}
...
QuicCryptoServerStreamBase* QuicNormalServerSessionBase::GetCryptoStream() {
	return crypto_stream_.get();
}

int32_t QuicNormalServerSessionBase::BandwidthToCachedParameterBytesPerSecond(
	const QuicBandwidth& bandwidth) {
	int64_t bytes_per_second = bandwidth.ToBytesPerSecond();
	return (bytes_per_second > static_cast<int64_t>(INT32_MAX)
		? INT32_MAX
