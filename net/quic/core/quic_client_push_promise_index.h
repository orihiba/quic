...
class NET_EXPORT_PRIVATE QuicClientPushPromiseIndex2 {
public:
	// Delegate is used to complete the rendezvous that began with
	// |Try()|.
	class NET_EXPORT_PRIVATE Delegate {
	public:
		...
		// |Try()| will return |QUIC_FAILURE|, which may be convenient for
		// retry purposes.
		virtual void OnRendezvousResult(QuicNormalStream* stream) = 0;
	};
	...
	QuicClientPushPromiseIndex2();
	virtual ~QuicClientPushPromiseIndex2();

	// Called by client code, used to enforce affinity between requests
	// for promised streams and the session the promise came from.
	QuicClientPromisedInfo2* GetPromised(const std::string& url);
	...
	QuicPromisedByUrlMap2* promised_by_url() { return &promised_by_url_; }

private:
	QuicPromisedByUrlMap2 promised_by_url_;

	DISALLOW_COPY_AND_ASSIGN(QuicClientPushPromiseIndex2);
};
