...
public:
	...
	QuicDispatcher2(const QuicConfig& config,
		const QuicCryptoServerConfig* crypto_config,
		QuicVersionManager* version_manager,
		std::unique_ptr<QuicConnectionHelperInterface> helper,
		std::unique_ptr<QuicCryptoServerStream::Helper> session_helper,
	...
	void OnPacketBeingDispatchedToSession(
		QuicNormalServerSessionBase* session) override {}

	typedef std::unordered_map<QuicConnectionId, QuicNormalServerSessionBase*>
		SessionMap;
...
protected:
	...
	// Create and return the time wait list manager for this dispatcher, which
	// will be owned by the dispatcher as time_wait_list_manager_
	virtual QuicTimeWaitListManager2* CreateQuicTimeWaitListManager();
...
private:
	friend class net::test::QuicDispatcherPeer;
	friend class StatelessRejectorProcessDoneCallback2;
	...
	// Entity that manages connection_ids in time wait state.
	std::unique_ptr<QuicTimeWaitListManager2> time_wait_list_manager_;

	// The list of closed but not-yet-deleted sessions.
	std::vector<QuicNormalServerSessionBase*> closed_session_list_;
