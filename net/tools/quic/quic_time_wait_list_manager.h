...
class QuicTimeWaitListManager2 : public QuicBlockedWriterInterface {
public:
	// writer - the entity that writes to the socket. (Owned by the dispatcher)
	// visitor - the entity that manages blocked writers. (The dispatcher)
	// helper - provides a clock (Owned by the dispatcher)
	// alarm_factory - used to run clean up alarms. (Owned by the dispatcher)
	QuicTimeWaitListManager2(QuicPacketWriter* writer,
		QuicNormalServerSessionBase::Visitor* visitor,
		QuicConnectionHelperInterface* helper,
		QuicAlarmFactory* alarm_factory);
	~QuicTimeWaitListManager2() override;
...
private:
	...
	// Interface that manages blocked writers.
	QuicNormalServerSessionBase::Visitor* visitor_;

	DISALLOW_COPY_AND_ASSIGN(QuicTimeWaitListManager2);
};
