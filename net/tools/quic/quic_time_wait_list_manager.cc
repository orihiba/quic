...
class ConnectionIdCleanUpAlarm2 : public QuicAlarm::Delegate {
public:
	explicit ConnectionIdCleanUpAlarm2(
...
private:
	...
	DISALLOW_COPY_AND_ASSIGN(ConnectionIdCleanUpAlarm2);
};
...
class QuicTimeWaitListManager2::QueuedPacket {
public:
	QueuedPacket(const IPEndPoint& server_address,
		const IPEndPoint& client_address,
		QuicEncryptedPacket* packet)
...
QuicTimeWaitListManager2::QuicTimeWaitListManager2(
	QuicPacketWriter* writer,
	QuicNormalServerSessionBase::Visitor* visitor,
	QuicConnectionHelperInterface* helper,
	QuicAlarmFactory* alarm_factory)
	: time_wait_period_(
		QuicTime::Delta::FromSeconds(FLAGS_quic_time_wait_list_seconds)),
	connection_id_clean_up_alarm_(
		alarm_factory->CreateAlarm(new ConnectionIdCleanUpAlarm2(this))),
	clock_(helper->GetClock()),
	writer_(writer),
	visitor_(visitor) {
	SetConnectionIdCleanUpAlarm();
...
void QuicTimeWaitListManager2::AddConnectionIdToTimeWait(
	QuicConnectionId connection_id,
	QuicVersion version,
	bool connection_rejected_statelessly,
	std::vector<std::unique_ptr<QuicEncryptedPacket>>* termination_packets) {
...
bool QuicTimeWaitListManager2::IsConnectionIdInTimeWait(
	QuicConnectionId connection_id) const {
	return base::ContainsKey(connection_id_map_, connection_id);
}

QuicVersion QuicTimeWaitListManager2::GetQuicVersionFromConnectionId(
	QuicConnectionId connection_id) {
	ConnectionIdMap::iterator it = connection_id_map_.find(connection_id);
	DCHECK(it != connection_id_map_.end());
	return (it->second).version;
...
void QuicTimeWaitListManager2::OnCanWrite() {
	while (!pending_packets_queue_.empty()) {
		QueuedPacket* queued_packet = pending_packets_queue_.front();
		if (!WriteToWire(queued_packet)) {
			return;
...
void QuicTimeWaitListManager2::ProcessPacket(
	const IPEndPoint& server_address,
	const IPEndPoint& client_address,
	QuicConnectionId connection_id,
	QuicPacketNumber packet_number,
...
void QuicTimeWaitListManager2::SendVersionNegotiationPacket(
	QuicConnectionId connection_id,
	const QuicVersionVector& supported_versions,
	const IPEndPoint& server_address,
	const IPEndPoint& client_address) {
...
// power of 2 to throttle the number of public reset packets we send to a
// client.
bool QuicTimeWaitListManager2::ShouldSendResponse(int received_packet_count) {
	return (received_packet_count & (received_packet_count - 1)) == 0;
}

void QuicTimeWaitListManager2::SendPublicReset(
	const IPEndPoint& server_address,
	const IPEndPoint& client_address,
	QuicConnectionId connection_id,
	QuicPacketNumber rejected_packet_number) {
...
QuicEncryptedPacket* QuicTimeWaitListManager2::BuildPublicReset(
	const QuicPublicResetPacket& packet) {
	return QuicFramer::BuildPublicResetPacket(packet);
}
...
// Either sends the packet and deletes it or makes pending queue the
// owner of the packet.
void QuicTimeWaitListManager2::SendOrQueuePacket(QueuedPacket* packet) {
	if (WriteToWire(packet)) {
		delete packet;
	}
	else {
...
bool QuicTimeWaitListManager2::WriteToWire(QueuedPacket* queued_packet) {
	if (writer_->IsWriteBlocked()) {
		visitor_->OnWriteBlocked(this);
		return false;
	}
...
void QuicTimeWaitListManager2::SetConnectionIdCleanUpAlarm() {
	QuicTime::Delta next_alarm_interval = QuicTime::Delta::Zero();
	if (!connection_id_map_.empty()) {
		QuicTime oldest_connection_id =
			connection_id_map_.begin()->second.time_added;
...
bool QuicTimeWaitListManager2::MaybeExpireOldestConnection(
	QuicTime expiration_time) {
	if (connection_id_map_.empty()) {
		return false;
	}
...
void QuicTimeWaitListManager2::CleanUpOldConnectionIds() {
	QuicTime now = clock_->ApproximateNow();
	QuicTime expiration = now - time_wait_period_;
...
void QuicTimeWaitListManager2::TrimTimeWaitListIfNeeded() {
	if (FLAGS_quic_time_wait_list_max_connections < 0) {
		return;
	}
	while (num_connections() >=
...
QuicTimeWaitListManager2::ConnectionIdData::ConnectionIdData(
	int num_packets_,
	QuicVersion version_,
	QuicTime time_added_,
	bool connection_rejected_statelessly)
