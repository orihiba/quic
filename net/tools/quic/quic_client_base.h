...
class QuicNormalClientBase : public QuicClientPushPromiseIndex2::Delegate,
	public QuicNormalStream::Visitor {
public:
	// A ResponseListener is notified when a complete response is received.
	class ResponseListener {
	public:
	...
	QuicNormalClientBase(const QuicServerId& server_id,
		const QuicVersionVector& supported_versions,
		const QuicConfig& config,
		QuicConnectionHelperInterface* helper,
		QuicAlarmFactory* alarm_factory,
	...
	// QuicNormalStream::Visitor
	void OnClose(QuicNormalStream* stream) override;
	...
	// Returns a newly created QuicSpdyClientStream, owned by the
	// QuicSimpleClient.
	virtual QuicNormalStream* CreateReliableClientStream();
	...
	QuicNormalClientSession* session() { return session_.get(); }
	...
	void set_session(QuicNormalClientSession* session) { session_.reset(session); }
	...
	bool CheckVary(const SpdyHeaderBlock& client_request,
		const SpdyHeaderBlock& promise_request,
		const SpdyHeaderBlock& promise_response) override;
	void OnRendezvousResult(QuicNormalStream*) override;
...
protected:
	...
	// Takes ownership of |connection|.
	virtual QuicNormalClientSession* CreateQuicClientSession(
		QuicConnection* connection);
...
private:
	...
	// Session which manages streams.
	std::unique_ptr<QuicNormalClientSession> session_;
