...
void QuicDispatcher::OnFecProtectedPayload(base::StringPiece payload) {}
void QuicDispatcher::OnFecData(base::StringPiece redundnancy) {}
...
// An alarm that informs the QuicDispatcher to delete old sessions.
class DeleteSessionsAlarm2 : public QuicAlarm::Delegate {
public:
	explicit DeleteSessionsAlarm2(QuicDispatcher2* dispatcher)
		: dispatcher_(dispatcher) {}
...
private:
	// Not owned.
	QuicDispatcher2* dispatcher_;

	DISALLOW_COPY_AND_ASSIGN(DeleteSessionsAlarm2);
};

class StatelessConnectionTerminator2 {
public:
	StatelessConnectionTerminator2(QuicConnectionId connection_id,
		QuicFramer* framer,
		QuicConnectionHelperInterface* helper,
		QuicTimeWaitListManager2* time_wait_list_manager)
		: connection_id_(connection_id),
		framer_(framer),
		creator_(connection_id,
			framer,
...
QuicDispatcher2::QuicDispatcher2(
	...
	alarm_factory_(std::move(alarm_factory)),
	delete_sessions_alarm_(
		alarm_factory_->CreateAlarm(new DeleteSessionsAlarm2(this))),
	buffered_packets_(this, helper_->GetClock(), alarm_factory_.get()),
	current_packet_(nullptr),
	version_manager_(version_manager),
	framer_(GetSupportedVersions(),
...
void QuicDispatcher2::InitializeWithWriter(QuicPacketWriter* writer) {
	DCHECK(writer_ == nullptr);
	writer_.reset(writer);
	time_wait_list_manager_.reset(CreateQuicTimeWaitListManager());
}

void QuicDispatcher2::ProcessPacket(const IPEndPoint& server_address,
	const IPEndPoint& client_address,
	const QuicReceivedPacket& packet) {
	current_server_address_ = server_address;
	current_client_address_ = client_address;
...
bool QuicDispatcher2::OnUnauthenticatedPublicHeader(
	const QuicPacketPublicHeader& header) {
	current_connection_id_ = header.connection_id;
...
bool QuicDispatcher2::OnUnauthenticatedHeader(const QuicPacketHeader& header) {
	QuicConnectionId connection_id = header.public_header.connection_id;
...
void QuicDispatcher2::ProcessUnauthenticatedHeaderFate(
	QuicPacketFate fate,
	QuicConnectionId connection_id,
	QuicPacketNumber packet_number) {
	switch (fate) {
...
QuicDispatcher2::QuicPacketFate QuicDispatcher2::ValidityChecks(
	const QuicPacketHeader& header) {
	// To have all the checks work properly without tears, insert any new check
	// into the framework of this method in the section for checks that return the
	// check's fate value.  The sections for checks must be ordered with the
...
void QuicDispatcher2::CleanUpSession(SessionMap::iterator it,
	bool should_close_statelessly) {
	QuicConnection* connection = it->second->connection();
...
void QuicDispatcher2::DeleteSessions() {
	base::STLDeleteElements(&closed_session_list_);
}

void QuicDispatcher2::OnCanWrite() {
	// The socket is now writable.
	writer_->SetWritable();
...
bool QuicDispatcher2::HasPendingWrites() const {
	return !write_blocked_list_.empty();
}

void QuicDispatcher2::Shutdown() {
	while (!session_map_.empty()) {
		QuicNormalServerSessionBase* session = session_map_.begin()->second;
		session->connection()->CloseConnection(
			QUIC_PEER_GOING_AWAY, "Server shutdown imminent",
			ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
		// Validate that the session removes itself from the session map on close.
...
void QuicDispatcher2::OnConnectionClosed(QuicConnectionId connection_id,
	QuicErrorCode error,
	const string& error_details) {
	SessionMap::iterator it = session_map_.find(connection_id);
	if (it == session_map_.end()) {
...
void QuicDispatcher2::OnWriteBlocked(
	QuicBlockedWriterInterface* blocked_writer) {
	if (!writer_->IsWriteBlocked()) {
		QUIC_BUG << "QuicDispatcher2::OnWriteBlocked called when the writer is "
			"not blocked.";
		// Return without adding the connection to the blocked list, to avoid
		// infinite loops in OnCanWrite.
		return;
...
void QuicDispatcher2::OnConnectionAddedToTimeWaitList(
	QuicConnectionId connection_id) {
	DVLOG(1) << "Connection " << connection_id << " added to time wait list.";
}
...
void QuicDispatcher2::OnFecData(base::StringPiece redundnancy) {}

void QuicDispatcher2::OnError(QuicFramer* framer) {
	QuicErrorCode error = framer->error();
	SetLastError(error);
	DVLOG(1) << QuicUtils::ErrorToString(error);
}

bool QuicDispatcher2::ShouldCreateSessionForUnknownVersion(QuicTag version_tag) {
	return false;
}

bool QuicDispatcher2::OnProtocolVersionMismatch(
	QuicVersion /*received_version*/) {
	QUIC_BUG_IF(!time_wait_list_manager_->IsConnectionIdInTimeWait(
		current_connection_id_) &&
		!ShouldCreateSessionForUnknownVersion(framer_.last_version_tag()))
...
void QuicDispatcher2::OnPublicResetPacket(
	const QuicPublicResetPacket& /*packet*/) {
	DCHECK(false);
}
void QuicDispatcher2::OnRevivedPacket() {
	DCHECK(false);
}
void QuicDispatcher2::OnVersionNegotiationPacket(
	const QuicVersionNegotiationPacket& /*packet*/) {
	DCHECK(false);
}

void QuicDispatcher2::OnDecryptedPacket(EncryptionLevel level) {
	DCHECK(false);
}

bool QuicDispatcher2::OnPacketHeader(const QuicPacketHeader& /*header*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnStreamFrame(const QuicStreamFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnAckFrame(const QuicAckFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnStopWaitingFrame(const QuicStopWaitingFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnPaddingFrame(const QuicPaddingFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnPingFrame(const QuicPingFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnRstStreamFrame(const QuicRstStreamFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnConnectionCloseFrame(
	const QuicConnectionCloseFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnGoAwayFrame(const QuicGoAwayFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnWindowUpdateFrame(
	const QuicWindowUpdateFrame& /*frame*/) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnBlockedFrame(const QuicBlockedFrame& frame) {
	DCHECK(false);
	return false;
}

bool QuicDispatcher2::OnPathCloseFrame(const QuicPathCloseFrame& frame) {
	DCHECK(false);
	return false;
}

void QuicDispatcher2::OnPacketComplete() {
	DCHECK(false);
}

void QuicDispatcher2::OnExpiredPackets(
	QuicConnectionId connection_id,
	BufferedPacketList early_arrived_packets) {
	time_wait_list_manager_->AddConnectionIdToTimeWait(
		connection_id, framer_.version(), false, nullptr);
...
void QuicDispatcher2::ProcessBufferedChlos(size_t max_connections_to_create) {
	// Reset the counter before starting creating connections.
	new_sessions_allowed_per_event_loop_ = max_connections_to_create;
	for (; new_sessions_allowed_per_event_loop_ > 0;
		--new_sessions_allowed_per_event_loop_) {
...
bool QuicDispatcher2::HasChlosBuffered() const {
	return buffered_packets_.HasChlosBuffered();
}

bool QuicDispatcher2::ShouldCreateOrBufferPacketForConnection(
	QuicConnectionId connection_id) {
	VLOG(1) << "Received packet from new connection " << connection_id;
	return true;
}
...
// Return true if there is any packet buffered in the store.
bool QuicDispatcher2::HasBufferedPackets(QuicConnectionId connection_id) {
	return buffered_packets_.HasBufferedPackets(connection_id);
}

void QuicDispatcher2::OnBufferPacketFailure(EnqueuePacketResult result,
	QuicConnectionId connection_id) {
	DVLOG(1) << "Fail to buffer packet on connection " << connection_id
		<< " because of " << result;
}

void QuicDispatcher2::OnConnectionRejectedStatelessly() {}

void QuicDispatcher2::OnConnectionClosedStatelessly(QuicErrorCode error) {}

bool QuicDispatcher2::ShouldAttemptCheapStatelessRejection() {
	return true;
}

QuicTimeWaitListManager2* QuicDispatcher2::CreateQuicTimeWaitListManager() {
	return new QuicTimeWaitListManager2(writer_.get(), this, helper_.get(),
		alarm_factory_.get());
}

void QuicDispatcher2::BufferEarlyPacket(QuicConnectionId connection_id) {
	bool is_new_connection = !buffered_packets_.HasBufferedPackets(connection_id);
	if (FLAGS_quic_create_session_after_insertion && is_new_connection &&
		!ShouldCreateOrBufferPacketForConnection(connection_id)) {
		return;
...
void QuicDispatcher2::ProcessChlo() {
	QUIC_BUG_IF(!FLAGS_quic_buffer_packet_till_chlo &&
		FLAGS_quic_limit_num_new_sessions_per_epoll_loop)
		<< "Try to limit connection creation per epoll event while not "
		"supporting packet buffer. "
	...
	session->SetMaxDelay(server_->max_delay());
	lostBytesDelta = server_->lost_bytes_delta();
	DVLOG(1) << "Created new session for " << current_connection_id_;
	session_map_.insert(std::make_pair(current_connection_id_, session));
	std::list<BufferedPacket> packets =
		buffered_packets_.DeliverPackets(current_connection_id_);
...
QuicPacketWriter* QuicDispatcher2::CreatePerConnectionWriter() {
	return new QuicPerConnectionPacketWriter(writer_.get());
}

void QuicDispatcher2::SetLastError(QuicErrorCode error) {
	last_error_ = error;
}

bool QuicDispatcher2::OnUnauthenticatedUnknownPublicHeader(
	const QuicPacketPublicHeader& header) {
	return true;
}

class StatelessRejectorProcessDoneCallback2
	: public StatelessRejector::ProcessDoneCallback {
public:
	StatelessRejectorProcessDoneCallback2(QuicDispatcher2* dispatcher,
		QuicPacketNumber packet_number,
		QuicVersion first_version)
		: dispatcher_(dispatcher),
		packet_number_(packet_number),
...
void QuicDispatcher2::MaybeRejectStatelessly(QuicConnectionId connection_id,
	const QuicPacketHeader& header) {
	// TODO(rch): This logic should probably live completely inside the rejector.
	if (!FLAGS_quic_use_cheap_stateless_rejects ||
		!FLAGS_enable_quic_stateless_reject_support ||
	...
	if (!validator.can_accept()) {
		// This CHLO is prohibited by policy.
		StatelessConnectionTerminator2 terminator(connection_id, &framer_, helper(),
			time_wait_list_manager_.get());
		terminator.CloseConnection(QUIC_HANDSHAKE_FAILED,
			validator.error_details());
		OnConnectionClosedStatelessly(QUIC_HANDSHAKE_FAILED);
	...
	// Continue stateless rejector processing
	std::unique_ptr<StatelessRejectorProcessDoneCallback2> cb(
		new StatelessRejectorProcessDoneCallback2(
			this, header.packet_number, header.public_header.versions.front()));
	StatelessRejector::Process(std::move(rejector), std::move(cb));
}

void QuicDispatcher2::OnStatelessRejectorProcessDone(
	std::unique_ptr<StatelessRejector> rejector,
	QuicPacketNumber packet_number,
	QuicVersion first_version) {
	QuicPacketFate fate;
	...
	case StatelessRejector::FAILED: {
		// There was an error processing the client hello.
		StatelessConnectionTerminator2 terminator(rejector->connection_id(),
			&framer_, helper(),
			time_wait_list_manager_.get());
		terminator.CloseConnection(rejector->error(), rejector->error_details());
		fate = kFateTimeWait;
...
const QuicVersionVector& QuicDispatcher2::GetSupportedVersions() {
	return version_manager_->GetSupportedVersions();
}

void QuicDispatcher2::DeliverPacketsToSession(
	const std::list<BufferedPacket>& packets,
	QuicNormalServerSessionBase* session) {
	for (const BufferedPacket& packet : packets) {
		session->ProcessUdpPacket(packet.server_address, packet.client_address,
