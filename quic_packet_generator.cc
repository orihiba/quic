...
QuicConsumedData QuicPacketGenerator::ConsumeData(
  ...
  while (delegate_->ShouldGeneratePacket(
    ...
    if (!InBatchMode()) {
      packet_creator_.Flush();
	  packet_creator_.MaybeSendFecPacketAndCloseGroup(/*force=*/false, /*is_fec_timeout=*/false); // HIBA not sure if needed
    }
...
void QuicPacketGenerator::SendQueuedFrames(bool flush) {
 ...
 // packet_creator_.MaybeSendFecPacketAndCloseGroup(false, false); // HIBA - in original was flush instead of first false
}
